...
## ⚠️ Важно для AI Assistants и разработчиков

**Перед началом работы с проектом:**
1. **ОБЯЗАТЕЛЬНО прочитайте `docs/AI_MEMORY.md`** - это источник истины для всех UI/UX правил и соглашений проекта
2. Изучите `docs/PROJECT_STATE.md` для понимания текущего состояния проекта и последних изменений
3. При внесении изменений в UI/UX или правила проекта - **ОБЯЗАТЕЛЬНО обновите `AI_MEMORY.md`**

**AI_MEMORY.md содержит:**
- Стабильные правила UI/UX, согласованные с владельцем проекта
- Правила дизайна и взаимодействия
- Источник истины для будущих изменений

**Обновление AI_MEMORY.md требуется при:**
- Изменении UI/UX правил
- Добавлении новых соглашений по дизайну
- Изменении поведения компонентов
- Обновлении правил работы с формами, кнопками, навигацией
...
### Вариант 3: Netlify (serverless)

Подходит для деплоя монолита как serverless приложения. Frontend и API работают через Netlify Functions.

**Преимущества:**
- Автоматический деплой из GitHub
- Бесплатный тариф для старта
- Встроенная поддержка serverless functions
- Scheduled functions для cron задач

**Ограничения:**
- Cold start для функций (первый запрос может быть медленным)
- Таймауты: 10 секунд (free), 26 секунд (pro)
- Минимальный интервал для scheduled functions: 1 минута
...
4. Настрой Scheduled Functions (опционально):
   - Functions → Scheduled functions
   - Добавь `scheduled-check` с расписанием `* * * * *`

**Подробная инструкция:** См. [NETLIFY_QUICKSTART.md](NETLIFY_QUICKSTART.md) или [docs/NETLIFY_DEPLOY.md](docs/NETLIFY_DEPLOY.md)

---

### Сравнение вариантов

| Характеристика | VPS (монолит) | Vercel + Backend | Netlify (serverless) |
|---|---|---|---|
| Простота настройки | ⭐⭐⭐ Средняя | ⭐⭐ Сложная | ⭐⭐⭐⭐ Простая |
| Стоимость | ⭐⭐⭐ От $5/мес | ⭐⭐⭐⭐ Vercel бесплатно, backend от $5/мес | ⭐⭐⭐⭐ Бесплатно для старта |
| Cron поддержка | ✅ Да (встроено) | ❌ Нет (нужен внешний) | ⚠️ Ограниченная (min 1 мин) |
| Масштабирование | ⭐⭐ Ручное | ⭐⭐⭐⭐ Автоматическое | ⭐⭐⭐⭐ Автоматическое |
| Производительность | ⭐⭐⭐⭐ Высокая | ⭐⭐⭐ Хорошая | ⭐⭐⭐ Хорошая (cold start) |
| Рекомендация | Для старта и малого трафика | Для production с высоким трафиком | Для быстрого старта и малого/среднего трафика |
