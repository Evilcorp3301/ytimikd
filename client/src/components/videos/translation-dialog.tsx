...
export function TranslationDialog({
  ...
  const [_userModifiedVoiceGender, setUserModifiedVoiceGender] = useState(false);
  const [_userModifiedVoiceName, setUserModifiedVoiceName] = useState(false);
  const [showVoiceOver, setShowVoiceOver] = useState(
    Boolean(translation?.voiceOverName || translation?.voiceOverGender)
  );
  ...
  useEffect(() => {
    if (open) {
      ...
      setShowVoiceOver(Boolean(translation?.voiceOverName || translation?.voiceOverGender));
      form.reset({
        translatedUrl: translation?.translatedUrl || "",
        publishMode:
          translation?.scheduledDate && new Date(translation.scheduledDate).getTime() > Date.now()
        ...
        scheduledTime: getTimeFromDate(translation?.scheduledDate),
        status:
          (translation?.status as "not_started" | "in_progress" | "completed") || "not_started",
      });
    }
  }, [open, translation]);
  ...
  useEffect(() => {
    if (watchedChannelId && watchedChannelId !== lastAppliedChannel) {
      ...
      if (selectedChannel) {
        // Автозаполнение работает независимо от состояния Switch "Озвучка"
        // Всегда заполняем поля из канала при выборе канала
  ...
  }, [watchedChannelId, channels, lastAppliedChannel, form]);
  ...
  const handleSubmit = (values: TranslationFormValues) => {
    ...
    // Type assertion needed because we remove scheduledTime and publishMode from the form values
    // but the backend API accepts the remaining fields
    onSave({ ...rest, channelId, scheduledDate: finalScheduledDate } as TranslationFormValues);
  };
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-md" data-testid="dialog-translation">
        <DialogHeader>
          <DialogTitle data-testid="text-dialog-title">Обновить детали перевода</DialogTitle>
        </DialogHeader>
        <DialogBody>
          <Form {...form}>
            <form onSubmit={form.handleSubmit(handleSubmit)} className="space-y-5">
                    ...
                    <FormItem>
                      ...
                      {videoSubcategoryId && (
                        <p className="text-xs text-muted-foreground">
                          Показаны только каналы, подходящие для подкатегории видео и языка{" "}
                          {language}
                        </p>
                      )}
                      <FormMessage />
              ...
              <div className="space-y-4 p-4 rounded-lg bg-muted/30 border border-border/40">
                <FormItem className="flex items-center justify-between rounded-lg border p-3 space-y-0">
                  <FormLabel className="mb-0">Озвучка</FormLabel>
                  <FormControl>
                    <Switch
                      checked={showVoiceOver}
                      onCheckedChange={(checked) => {
                        setShowVoiceOver(checked);
                        // Switch только управляет видимостью, не очищает поля
                      }}
