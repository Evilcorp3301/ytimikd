...
export class DatabaseStorage implements IStorage {
  ...
  async getActivityLogs(
    ...
    // Десериализуем metadata из JSON строки обратно в объект
  ...
  async getSettings(): Promise<Settings[]> {
    ...
    // Десериализуем value из JSON строки обратно в объект
  ...
  async upsertSetting(key: string, value: unknown): Promise<Settings> {
    // Сериализуем value в JSON строку для SQLite
    ...
    if (existing) {
      const [result] = await db
        ...
        .where(eq(settings.key, key))
        .returning();
      // Десериализуем value обратно в объект
      return {
        ...result,
        value: JSON.parse(result.value as string),
      };
    }
    const [result] = await db.insert(settings).values({ key, value: serializedValue }).returning();
    // Десериализуем value обратно в объект
    return {
      ...result,
      value: JSON.parse(result.value as string),
  ...
  async search(query: string): Promise<{
    ...
    // SQLite: используем UPPER() для case-insensitive поиска вместо ILIKE
    const videoConditions = or(
      sql`UPPER(${videos.url}) LIKE UPPER(${searchPattern})`,
      sql`UPPER(${videos.id}) LIKE UPPER(${searchPattern})`,
      sql`UPPER(COALESCE(${videos.title}, '')) LIKE UPPER(${searchPattern})`
    );
    ...
    const channelConditions = or(
      sql`UPPER(${channels.url}) LIKE UPPER(${searchPattern})`,
      sql`UPPER(${channels.id}) LIKE UPPER(${searchPattern})`,
      sql`UPPER(COALESCE(${channels.name}, '')) LIKE UPPER(${searchPattern})`
    );
