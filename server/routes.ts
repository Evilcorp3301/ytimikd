...
export async function registerRoutes(httpServer: Server, app: Express): Promise<Server> {
  ...
  app.get("/api/channels/stats", async (req, res) => {
    try {
      ...
      // Получаем количество опубликованных переводов для каждого канала
      const stats = await db
        ...
        .from(translations)
        .where(
          sql`${translations.status} = 'completed' AND ${translations.publishedDate} IS NOT NULL`
        )
        .groupBy(translations.channelId);

      const statsMap: Record<string, number> = {};
      stats.forEach((stat) => {
        if (stat.channelId) {
          statsMap[stat.channelId] = Number(stat.count);
        }
      });
  ...
  app.get("/api/translations", async (req, res) => {
    try {
      ...
      const channelId =
        typeof req.query.channelId === "string" && req.query.channelId !== "all"
          ? req.query.channelId
          : undefined;
  ...
  app.patch("/api/translations/:id", async (req, res) => {
    try {
      ...
      const hasTranslatedUrl =
        typeof parsed.translatedUrl === "string" && parsed.translatedUrl.length > 0;
      if (hasTranslatedUrl) {
        const effectiveScheduled =
          (scheduledDateExplicitlyCleared
            ? undefined
            : (parsed.scheduledDate as Date | null | undefined)) ??
          (oldTranslation?.scheduledDate ? new Date(oldTranslation.scheduledDate) : undefined);
        const now = new Date();
        const scheduledInFuture = Boolean(
          effectiveScheduled && effectiveScheduled.getTime() > now.getTime()
        );
